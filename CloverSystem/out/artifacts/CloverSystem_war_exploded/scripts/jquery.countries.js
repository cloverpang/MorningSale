...
function print_country(country_id, selectedVal){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var option_str = document.getElementById(country_id);
	var x, i=0, opt;
	for(x in country_arr){
		option_str.options[i++] = opt = new Option(country_arr[x], country_arr[x]);
		
		if (selectedVal.toLowerCase() == opt.value.toLowerCase()) {
			opt.selected = 'selected';
		}
	}
}

function print_state(state_id, state_index, selectedVal){
	var option_str = document.getElementById(state_id);
	var x, i=0, opt; state_index++;
	var state_arr = s_a[state_index].split("|");
	for (x in state_arr) {
		option_str.options[i++] = opt = new Option(state_arr[x], state_arr[x]);
...
/* 
 * Modified by Morningstar 
 * Author : Naruemit Nasoke
 * License : Follows what's original license saying
 */
(function() {
    ...
    //Hot fix bug
    s_a.unshift('');
    s_a = $.map(s_a, function(n) {
        return '|' + n;
    });

    $.fn.countries = function(params) {
        var options = $.extend({
            state: '',
            selectedCountry: '',
            selectedState: ''
        ...
        var ct, st;
        for (ct in country_arr) {
            $(this).append($('<option/>', {
                value: country_arr[ct],
                text: country_arr[ct]
            }));
        }

        // Setup state dropdown if availble
        if (options.state) {
            // Binding change event
            $(this).change(function() {
                var ci, state_arr, st;
                ci = $.inArray($(this).val(), country_arr);

                if (ci > -1) {
                    // Clean up state dropdown
                    $(options.state).find('option').remove();

                    state_arr = s_a[++ci].split("|");
                    // Setup states dropdown
                    if ($.map(state_arr, function(n) { return n == '' }).length != 2) 
                    {
                        for (st in state_arr) 
                        {

                            $(options.state).append($('<option></option>', {
                                value: state_arr[st],
                                text: state_arr[st]
                            }));
                    ...
                    if (options.selectedState) {
                        $(options.state).val(options.selectedState);
                    }
                }
        ...
        if (options.selectedCountry) {
            $(this).val(options.selectedCountry).trigger('change');
        }

        // Return as the way jquery asked me to do 
        return this;
    }
